...
jira_helper_info() {
  ...
  if [ -d "${markdowns_dir}" ]; then
    ...
    echo "  Status: [OK] exists (${markdown_count} files)"
  ...
  if [ -f "${readme_path}" ]; then
    echo "README: ${readme_path} [OK]"
  else
    echo "README: ${readme_path} [X]"
  fi
  echo ""
  ...
  if [ -n "${ATLASSIAN_USER}" ]; then
    echo "  User: ${ATLASSIAN_USER} [OK]"
  else
    echo "  User: Not configured [X]"
  fi

  if [ -n "${ATLASSIAN_SITE_URL}" ]; then
    echo "  Site: ${ATLASSIAN_SITE_URL} [OK]"
  else
    echo "  Site: Not configured [X]"
  fi
  echo ""
...
self_update() {
  ...
  if cp "${script_dir}/${script_file}" "${backup_file}"; then
    ...
    echo "  [X] Failed to create backup"
    return 1
  fi
  echo ""
  ...
  if head -1 "${temp_file}" | grep -q "^#!/bin/bash"; then
    ...
    echo "  [OK] File appears to be a valid bash script"
  else
    echo "  [X] Downloaded file doesn't appear to be a bash script"
    echo ""
    echo "Restoring backup..."
    rm -f "${temp_file}"
    mv "${backup_file}" "${script_dir}/${script_file}"
...
cleanup_workspaces() {
  ...
  if [ "$removed" -gt 0 ]; then
    echo "[OK] Removed ${removed} stale workspace(s)"
  else
    echo "[OK] No stale workspaces found"
  fi
}
...
discover_workspaces() {
  ...
  if [ "$registered" -gt 0 ]; then
    echo "[OK] Registered ${registered} new workspace(s)"
    echo ""
...
EOF
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    _log "[OK] Comment added successfully"
    echo "$body" | jq -r '.id'
  else
    _log "[X] Failed to add comment (HTTP $http_code)"
    echo "$body" | jq '.' >&2
    return 1
  fi
}
...
EOF
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    _log "[OK] Comment updated successfully"
    echo "$body" | jq -r '.id'
  else
    _log "[X] Failed to update comment (HTTP $http_code)"
    echo "$body" | jq '.' >&2
    return 1
  fi
}
...
EOF
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    _log "[OK] Page created successfully: ${ATLASSIAN_SITE_URL}/wiki${web_url}"
    echo "$new_page_id"
  else
    _log "[X] Failed to create page (HTTP $http_code)"
    echo "$body" | jq '.' >&2
    return 1
  fi
}
...
update_confluence_page() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    _log "[OK] Page updated successfully to version $new_version"
    ...
    _log "[X] Failed to update page (HTTP $http_code)"
    echo "$body" | jq '.' >&2
    return 1
  fi
}
...
replace_confluence_page() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    local new_version
    new_version=$(echo "$body" | jq -r '.version.number')
    _log "[OK] Page replaced successfully to version $new_version"
    ...
    _log "[X] Failed to replace page (HTTP $http_code)"
    echo "$body" | jq '.' >&2
    return 1
  fi
}
...
create_jira_ticket() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    _log "[OK] Created ticket: ${ticket_key}"
    echo "$ticket_key"
    return 0
  else
    _log "[X] Failed to create ticket (HTTP $http_code)"
    jq '.' "$cache_file" >&2
    return 1
  fi
}
...
create_jira_subtask() {
  ...
  if [ "$http_code" -ge 200 ] && [ "$http_code" -lt 300 ]; then
    ...
    _log "[OK] Created sub-task: ${subtask_key}"
    echo "$subtask_key"
    return 0
  else
    _log "[X] Failed to create sub-task (HTTP $http_code)"
    jq '.' "$cache_file" >&2
    return 1
  fi
}
...
remediate() {
  ...
  echo "  [OK] Found ${ips:+IPs, }${ports:+ports, }${files:+file references}"
  echo ""
    ...
    for search_path in "${search_paths[@]}"; do
      if [ -d "$search_path" ]; then
        while IFS= read -r rbac_file; do
          ...
          echo "  [OK] Found RBAC config: $rbac_file"
      ...
      for rbac_file in "${rbac_files[@]}"; do
        ...
        if grep -q "secrets" "$rbac_file" 2>/dev/null; then
          ...
          if echo "$verbs" | grep -q "get.*list.*watch"; then
            analysis_findings+="  - [!] Grants broad read access (get, list, watch)\n"
          fi
        fi
      done
    ...
    for search_path in "${search_paths[@]}"; do
      if [ -d "$search_path" ]; then
        while IFS= read -r flux_file; do
          ...
          echo "  [OK] Found cluster config: $flux_file"
  ...
  echo "  [OK] Deep analysis complete"
  echo ""
  ...
  if [ ${#discovered_files[@]} -gt 0 ]; then
    ...
    echo "  [OK] Generated reviewer suggestions"
  else
...
- GitHub status: [OK] active, [X] suspended, Unknown = no GitHub match
...
EOF
  ...
  echo "  [OK] Document saved: $output_file"
  echo ""
  ...
  if [ -n "$cluster_name" ]; then
    echo "[OK] Cluster identified: $cluster_name"
  fi
  if [ ${#discovered_files[@]} -gt 0 ]; then
    echo "[OK] Files analyzed: ${#discovered_files[@]}"
  fi
  if [ -n "$analysis_findings" ]; then
    echo "[OK] RBAC analysis complete"
  fi
  if [ -n "$remediation_steps" ]; then
    echo "[OK] Remediation plan generated"
  fi
  echo ""
...
suggest_reviewers() {
  ...
  while IFS='|' read -r email name score commits; do
    ...
    if [ -n "$gh_user" ]; then
      ...
      if [ -z "$suspended" ]; then
        gh_status="[OK] ${gh_user}"
      else
        gh_status="[X] ${gh_user} (suspended)"
      fi
    fi
  ...
  echo "  - GitHub status: [OK] active, [X] suspended, Unknown = no GitHub match"
  echo ""
